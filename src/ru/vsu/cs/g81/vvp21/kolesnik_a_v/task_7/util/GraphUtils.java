package ru.vsu.cs.g81.vvp21.kolesnik_a_v.task_7.util;


import guru.nidi.graphviz.model.Link;
import guru.nidi.graphviz.model.MutableGraph;
import guru.nidi.graphviz.parse.Parser;
import ru.vsu.cs.g81.vvp21.kolesnik_a_v.task_7.Graph;
import ru.vsu.cs.g81.vvp21.kolesnik_a_v.task_7.Pair;
...
public class GraphUtils {
    ...
    public static Graph fromStr(String str, Class clz) throws IOException, InstantiationException, IllegalAccessException {
        ...
        if (Pattern.compile("^\\s*(strict\\s+)?(graph|digraph)\\s*\\{").matcher(str).find()) {
            ...
            for (int i = 0; i < edgeCount; i++) {
                graph.removeEdge(scanner.nextInt(), scanner.nextInt());
            }
        }*/ else {
            Scanner scanner = new Scanner(str);
            vertexCount = scanner.nextInt();
    ...
    public static String toDot(Graph graph) {
        ...
        for (int v1 = 0; v1 < graph.vertexCount(); v1++) {
            int count = 0;
            for (Integer v2 : graph.adjacenciesWith(v1)) {
    ...
    public static String toDotWithNew(Graph graph, List<Pair<Integer, Integer>> p) {
        StringBuilder sb = new StringBuilder();
        String nl = System.getProperty("line.separator");
        boolean isDigraph = false;
        sb.append("strict graph").append(" {").append(nl);
        ...
        for(Pair<Integer, Integer> n : p){
            sb.append(n.getKey()).append("--").append(n.getValue()).append("[style=dashed color=green]").append(nl);
        }
        sb.append("}").append(nl);
        return sb.toString();
    }
